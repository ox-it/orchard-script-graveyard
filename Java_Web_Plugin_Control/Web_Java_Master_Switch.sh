...
# Script to enable or disable the Oracle Java 7 Web Plugin by moving it between
# the /Library/Internet Plug-Ins/ folder and a 'disabled/' folder therein,
# intended to be made available via Casper Self Service. 
#
# Leaves a breadcrumb file to indicate enabled status (the default for MMP
# systems is disabled), for use by other scripts including the post-install
# script for Oracle Java 7 installs/updates, so that the current state of the
# plugin can be maintained. 
#
# Also creates/updates a symlink in /Library/Java/JavaVirtualMachines/ to the
# Java 7 plugin, since the plugin bundle contains the Oracle Java 7 JRE. This
# provides users with a stable location to place in their PATH, while the
# plugin itself may move.
...
VERSION=`/usr/libexec/PlistBuddy "${PLUGINS_DIR}/${JAVA_PLUGIN}/${PLIST_PATH}" -c 'print CFBundleVersion' 2>/dev/null`
...
if [ $PLUGIN_ENABLED -eq 0 ]; then
    ...
    VERSION=`/usr/libexec/PlistBuddy "${DISABLED_PLUGINS_DIR}/${JAVA_PLUGIN}/${PLIST_PATH}" -c 'print CFBundleVersion' 2>/dev/null`
...
if [ $PLUGIN_ENABLED -eq 0 ]; then
    ...
    if [ $CHOICE -eq 1 ]; then
        ...
        [ -e "${PLUGINS_DIR}/${JAVA_PLUGIN}" ] && /bin/rm -rf "${PLUGINS_DIR}/${JAVA_PLUGIN}"
